...
test("unified listing metadata is served from a fresh R2 cache", async () => {
  ...
  try {
    ...
    const cachedEnv = {
      MEDIA: {
        get: async (key) => {
          assert.equal(key, "api-cache/unified-listings-v5-source-aware-review.json");
          return {
...
test("memo edits commit directly to D1 without a Google runtime", async () => {
  ...
  try {
    ...
    assert.ok(!deletedCacheKeys.includes("api-cache/unified-listings-v5-source-aware-review.json"));
