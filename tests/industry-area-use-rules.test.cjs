...
assert.strictEqual(rules.industries["danran-bar"], "danran150");
assert.deepStrictEqual(
  rules.templates.danran150.bands.map((band) => band.condition),
  ["150㎡ 미만", "150㎡ 이상"]
);
assert.strictEqual(rules.industries["entertainment-bar"], "fixedSpecial");
