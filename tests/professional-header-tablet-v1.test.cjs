...
assert.match(html, /header-professional-v1\.css\?v=1\.1\.6-search-tell-layout/);
...
const labels = ["Tell", "검색", "필터", "초기화", "운영현황", "빠른등록", "로그아웃"];
let previous = -1;
...
assert.match(css, /grid-template-columns: 72px 0 84px 84px/);
assert.match(css, /\.desktop-operations-action \{ grid-column: 1/);
assert.match(css, /\.quick-add-btn \{ grid-column: 3/);
assert.match(css, /\.top-logout-btn-v1 \{ grid-column: 4/);
...
assert.match(css, /grid-template-columns: 84px 0 98px 98px/);
assert.match(css, /grid-template-columns: 96px 0 108px 108px/);
...
assert.match(tradeCss, /@media \(min-width: 1540px\)[\s\S]*grid-template-columns: 112px 48px minmax\(0, 1fr\) 76px !important/);
