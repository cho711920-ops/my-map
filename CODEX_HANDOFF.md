...
- 공간 클러스터 셀은 카카오 지도 단계별로 `level 6 이상 640m`, `5=320m`, `4=160m`, `3=80m`, `2=40m`이며 최대 확대 `level 1`은 같은 정확한 주소만 묶습니다. 확대할수록 한 셀이 더 작은 셀로 분리되므로 큰 클러스터가 자연스럽게 여러 개로 풀립니다.
...
- 자동 테스트는 전체 `253/253`, Cloudflare `124/124`, `cf:check`, `git diff --check`를 통과했습니다. 로그인된 운영 화면에서 새 엔진 클래스, 대전 5개 구 배치, 괴정동 검색, 당근 출처 필터, 찜폴더 5개 지도 반영, 상세→운영현황 전환, 기존 엔진 복귀를 직접 확인했습니다. 운영 자산 `map 8.2.7-world-grid-relayout`, `style 6.5.40-world-grid-clusters` 연결도 HTTP로 확인했습니다.
- 최종 운영 Worker 버전은 `536ead69-68c7-4a63-b3df-7a9d2c91e9c0`입니다. D1 스키마와 운영 데이터 변경은 없습니다.
