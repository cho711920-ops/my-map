...
- `level 8 구`, `level 7 동` 행정 클러스터는 기존처럼 전체 지역 합계를 사용합니다. `level 6 이하` 공간 클러스터만 현재 지도 목록과 정확히 같은 매물 집합을 세계 고정 격자에 배치하도록 분리했습니다. 세계 격자 키와 확대 단계별 크기는 유지되어 드래그 시 재버킷 현상은 다시 생기지 않습니다.
- 지도 자산은 `map.js?v=8.2.9-world-grid-viewport-count`입니다. `?clusterEngine=legacy` 안전 복구 경로는 그대로 유지됩니다.
...
- 행정구역 단계는 기본 대전 전체 화면 `level 8 이상=구 단위`, `level 7=동 단위`, `level 6 이하=고정 공간 클러스터`입니다. 행정 카드의 수량은 현재 검색·출처·구분·층·찜목록 필터 전체 결과를 기준으로 계산하고, 화면 밖 카드나 충돌하는 낮은 우선순위 카드는 다른 지역으로 멀리 밀지 않고 숨깁니다.
