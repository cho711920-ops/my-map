...
- 운영 적용 전 안전 조건 대상은 3,305건이었고, 저장 후보는 1개지만 같은 주소의 활성 대표매물이 여러 개인 약 680건은 제외했습니다. 배포 후 운영 화면에서 연속 처리를 시작했으며 완료 여부는 매물검증 건수와 `autoMergeSingleCandidate` 처리 결과로 확인할 수 있습니다.
- 전체 테스트 `246/246`, Cloudflare 테스트 `118/118`, 집중 테스트 `8/8`, `cf:check`, `git diff --check`를 통과했습니다. 운영 Worker 버전은 `72b4ca43-0c61-4eb9-b80f-8dff91660f2a`입니다.
