...
- 매물카드 선택 시 기존 `selectedItemKey → selectedGroupKey → redrawSelectedMarkers()` 연결 흐름은 변경하지 않고 시각 표현만 강화했다. 선택 카드에는 굵은 파란 왼쪽선·테두리·옅은 파란 배경을 표시하고, 같은 매물이 속한 일반·고정 공간·행정구역 클러스터도 파란색 링과 그림자로 강조한다. 다른 카드를 누르면 기존 단일 선택 상태를 따라 강조가 새 카드와 클러스터로 이동한다.
