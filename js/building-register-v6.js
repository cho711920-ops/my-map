(function () {
  ...
  function bindBadge(card, item) {
    ...
    if (persistentBadge || addressBadge) {
      ...
      // Cards only render the value already prepared in D1. Capacity lookup is
      // handled by the collector/scheduled Worker so entering the viewport
      // never starts an external API request or causes a late number flash.
      return;
    }
    var cachedParcel = readParcelCache(item);
    if (cachedParcel) {
