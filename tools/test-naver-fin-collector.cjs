...
(async () => {
  ...
  assert.strictEqual(api.version, "5.3.2");
  ...
  assert(source.includes("width:min(460px"));
  assert(source.includes("선택 클러스터 합계"));
  assert(source.includes("showNaverSaveWait"));
  assert(source.includes("고객매칭은 자동으로 별도 갱신"));
  ...
  assert(!source.includes('data-action="city"'));
  assert(source.includes("classifySourceManifest"));
  assert(source.includes('state.selectionMode = "cluster"'));
  assert(source.includes('state.selectedDistrict = null'));
  assert(source.includes('state.selectionMode === "district"'));
