...
데이터 품질 보류처럼 공개 목록 조회 조건을 바꾸는 마이그레이션은 반드시 `마이그레이션 → Worker 배포 → 데이터 보정` 순서로 적용한다. 보정 도구가 R2 캐시를 무효화하더라도 기존 Worker 격리 환경의 메모리 캐시는 최대 `SHEET_MEMORY_CACHE_MS` 동안 남을 수 있으므로, 보정 직후 Worker를 한 번 더 배포해 메모리 캐시를 비운 다음 운영 목록을 검증한다.
