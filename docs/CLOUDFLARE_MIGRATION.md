...
- 운영센터·찜목록·AI임장에 이어 `async-mutation-queue-v1.js`의 작업 접수와 상태 조회도 이 경계를 사용한다. 페이지 종료 저장의 `keepalive`, 로컬 outbox, 3초 재시도, 완료 이벤트 계약은 그대로 유지한다.
- 매물 CSV, 데이터 리비전·변경분, 통합매물 조회·원본 이동, 운영센터의 현황·고객·검증·이력·사용자 관리, 계정별 찜·임장목록과 AI 임장 진행상태·완료 메모는 이 모듈을 통해 Cloudflare API에 접근한다.
