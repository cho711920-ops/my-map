...
- 운영센터·찜목록·AI임장에 이어 `async-mutation-queue-v1.js`의 작업 접수·상태 조회, `diagnosis-storage.js`의 입점진단 저장·조회, `announcement-v1.js`의 활성 공지 조회도 이 경계를 사용한다. 화면에서 비활성 상태인 `listing-duplicate-cleanup-v1.js`의 수동 중복정리 저장 호출도 재활성화 없이 이 경계로 이관해 두었다. 페이지 종료 저장의 `keepalive`, 진단 저장의 5초 제한·로컬 복구, 비동기 큐의 로컬 outbox·3초 재시도·완료 이벤트, 공지 닫기·재표시 계약은 그대로 유지한다.
